...
class TideProxy:
    ...
    def __init__(self, hass, latitude: float, longitude: float, ttl: int = _DEFAULT_TTL):
        ...
        _LOGGER.debug(
            "TideProxy initialized, data_dir=%s (Skyfield resources will load lazily)",
            self._data_dir,
        )
    ...
    async def get_tide_for_timestamps(self, timestamps: Sequence[str]) -> Dict[str, Any]:
        ...
        try:
            moon_transit_dt = await self._async_find_next_moon_transit(
                sf_eph, sf_ts, sf_almanac, sf_wgs, now
            )
        except Exception:
        ...
        try:
            times_list = [
                sf_ts.utc(dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second) for dt in dt_objs
            ]
        except Exception:
        ...
        try:
            if moon_transit_dt is not None:
                ...
                t_anchor = sf_ts.utc(
                    moon_transit_dt.year,
                    moon_transit_dt.month,
                    moon_transit_dt.day,
                    moon_transit_dt.hour,
                    moon_transit_dt.minute,
                    moon_transit_dt.second,
                )
        ...
        if moon_transit_dt is None:
            ...
            _LOGGER.debug(
                "Using skyfield moon_transit_dt as anchor (no lon_shift applied) anchor_dt=%s",
                anchor_dt.isoformat().replace("+00:00", "Z"),
            )
    ...
    async def _async_find_next_moon_transit(
        self, sf_eph, sf_ts, sf_almanac, sf_wgs, start_dt: datetime
    ) -> Optional[datetime]:
        """
        ...
        for d in dates_needed:
            try:
                ...
                if mode == "dawn_dusk":
                    ...
                    t0_exp = sf_ts.utc(
                        (day_start - expand).year,
                        (day_start - expand).month,
                        (day_start - expand).day,
                        (day_start - expand).hour,
                        (day_start - expand).minute,
                        (day_start - expand).second,
                    )
                    t1_exp = sf_ts.utc(
                        (day_end + expand).year,
                        (day_end + expand).month,
                        (day_end + expand).day,
                        (day_end + expand).hour,
                        (day_end + expand).minute,
                        (day_end + expand).second,
                    )
                    ...
                    if not times:
                        raise RuntimeError(
                            f"No sunrise/sunset events found for date {d.isoformat()} at location lat={self.latitude},lon={self.longitude}"
                        )
                    ...
                    if not sunrise_candidates or not sunset_candidates:
                        ...
                        raise RuntimeError(
                            f"Unable to determine sunrise or sunset for date {d.isoformat()} at lat={self.latitude},lon={self.longitude}"
                        )
